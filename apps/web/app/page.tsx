...
async function createTask(formData: FormData) {
  "use server";
  const projectId = String(formData.get("projectId") ?? "").trim(); const title = String(formData.get("title") ?? "").trim(); const description = String(formData.get("description") ?? "").trim(); const priority = String(formData.get("priority") ?? "medium"); const assigneeId = String(formData.get("assigneeId") ?? "").trim(); const dueDateInput = String(formData.get("dueDate") ?? "").trim();
  if (!projectId || title.length < 3 || description.length < 5 || !["low", "medium", "high", "urgent"].includes(priority)) return;
  const response = await fetch(`${getApiUrl()}/tasks`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ projectId, title, description, priority, assigneeId: assigneeId || null, dueDate: dueDateInput ? new Date(dueDateInput).toISOString() : null }) });
