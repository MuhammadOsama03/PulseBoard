...
export default async function HomePage() {
  let projects = demoProjects; let sourceLabel = "Demo workspace"; let apiAvailable = false; let taskMetrics = demoTaskMetrics; let recentTasks: Task[] = [];
  try {
    const [apiProjects, apiTasks] = await Promise.all([loadProjects(), loadTasks()]); apiAvailable = true; projects = apiProjects; sourceLabel = apiProjects.length > 0 ? "Live API data" : "Live workspace - no projects yet";
    ...
    taskMetrics = { open: apiTasks.length - completed, inProgress, completed }; recentTasks = apiTasks.slice(0, 5);
